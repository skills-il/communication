...
5. **2G/3G wind-down (IN PROGRESS, not complete)** - on 1 February 2026 the operators vacated the first-GHz bands that carried 2G/3G, for reuse by 4G/5G. That was the FIRST STAGE only: the Ministry of Communications separated frequency-vacating from service closure, and the legacy networks continue to operate in reduced form on other frequencies. Dialling the emergency centres (100, 101, 102) from old handsets stays available until the end of 2028, and the Ministry intends to require a dedicated 2G emergency network until then. Coverage quality on old devices may degrade, so still confirm VoLTE support before recommending a SIM-only plan, but do NOT tell a user their feature phone, alarm panel or elevator emergency line has already stopped working.
...
- VoLTE and VoWiFi are now the default voice paths as the legacy networks wind down. Some older 4G-only phones (pre-2018 generic Android, pre-iPhone 7) may have spotty VoLTE on certain Israeli carriers; verify on the carrier's compatible-device list before recommending.
