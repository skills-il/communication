...
```bash
# האם היום יום שאין בו עבודה בישראל (יום טוב, ערב חג, או יום חופש לאומי)?
# חול המועד, חנוכה והצומות הקטנים הם ימי עבודה ומוחרגים.
...
- **אין להוסיף `mf=on`.** הוא מוסיף בדיוק חמישה צומות ל-2026 (תענית אסתר, תענית בכורות, צום תמוז, צום גדליה ועשרה בטבת) ו**כל החמישה ימי עבודה רגילים**. שני הצומות שבאמת עוצרים עבודה, יום כיפור ותשעה באב, בכלל לא נושאים `subcat: "fast"`; הם כבר מגיעים תחת `major`. ענף `subcat=="fast"` הוא או קוד מת בלי הדגל או שגוי פעיל איתו.
...
- `references/israeli-holiday-gate.md`: איך נגזרה שאילתת Hebcal של שלב 6, כולל המנייה המלאה של 2026 ומלכודות הפרמטרים. להיוועץ בו לפני שינוי מסנן החגים.
