...
**לפני שמצטטים מע"מ, לברר את מעמד העוסק.** לשאול פעם אחת ולזכור. **עוסק מורשה** גובה מע"מ ומוציא חשבונית מס. **עוסק פטור** (מחזור מתחת לתקרה השנתית) אסור לו לגבות מע"מ או להוציא חשבונית מס בכלל, והוא מוציא קבלה. `scripts/shekel-formatter.py` מניח עוסק מורשה, אז אין להריץ אותו במצב `--vat` עבור עוסק פטור, ואין להוסיף שורת מע"מ להצעת מחיר שלו. טעות בכיוון הזה גורמת למשתמש לגבות מס שאסור לו לגבות.
...
**לסנן לפי `subcat`, לא לפי `category`, ולדייק בפרמטרים.** שלושה דברים שוברים גרסה נאיבית של השאילתה הזו, וכל השלושה נבדקו מול ה-API החי על כל שנת 2026:

- **`category=="holiday"` חוסם יותר מדי.** עם `min=on`, ב-2 בפברואר 2026 חוזר `{"category":"holiday","subcat":"minor","title":"Tu BiShvat"}`, יום עבודה ישראלי רגיל לגמרי, וכך גם ל"ג בעומר וט"ו באב. צריך להוריד את `min=on` ולסנן לפי `subcat`.
- **צומות דורשים `mf=on`.** בלעדיו אין בפיד שום פריט צום, ולכן ענף `subcat=="fast"` הוא קוד מת. עם `mf=on` הפיד של 2026 מחזיר בדיוק חמישה: תענית אסתר, תענית בכורות, צום תמוז, צום גדליה ועשרה בטבת, כל אחד כ-`category: "holiday"` עם `subcat: "fast"`.
- **`mod=on` מערבב ימי חופש עם ימי עבודה.** חגים מודרניים מגיעים כ-`subcat: "modern"`, ו-14 הרשומות של 2026 כוללות את יום הזיכרון ויום העצמאות (ימי חופש אמיתיים) לצד יום השפה העברית, יום המשפחה, יום העלייה ויום השואה (ימי עבודה). בחירה בכל `modern` חוסמת ארבעה ימי עבודה רגילים; ויתור עליו לגמרי מאפשר שליחה ביום העצמאות. צריך להתאים את השניים לפי הכותרת, כמו בשאילתה למעלה.

ערב חג מגיע כ-`subcat: "major"` (2 במרץ 2026 מחזיר "Erev Purim") - לטפל בו כמו ביום שישי ולא לשלוח אחרי ~14:00 שעון ישראל. שימו לב ש-`nx=on` מוסיף רשומות ראש חודש תחת `category: "roshchodesh"` בלי `subcat` בכלל; אלה ימי עבודה, ולכן השאילתה למעלה פשוט משמיטה את הדגל.
