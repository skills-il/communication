...
הדגל `i=on` מצמצם את התוצאות למנהג ישראלי (יום אחד של יום טוב בפסח/סוכות במקום שניים). `cfg=json` לפלט JSON. ערב חג ומוצאי חג מוחזרים גם הם עם מפתחות `subcat` (`major` לחג, `roshchodesh` לראש חודש, וכו') - לטפל בערב חג כמו ביום שישי: לא לשלוח אחרי ~14:00 שעון ישראל.

### שלב 7: עמידה בחוק הספאם (סעיף 30א) ואימות bulk sender

לפני שמאוטמים שליחה ליותר ממספר בודד של נמענים, חייבים לעבור דרך שני שערים: חוק הספאם הישראלי וכללי ה-bulk sender של Gmail.

**חוק הספאם הישראלי (סעיף 30א לחוק התקשורת (בזק ושידורים), תשמ"ב-1982):**

- "דבר פרסומת" אסור לשליחה במייל, SMS, פקס או חיוג אוטומטי **בלי הסכמה מפורשת מראש בכתב** של הנמען. ההסכמה צריכה להיות פעולה אקטיבית (תיבת opt-in לא מסומנת מראש או טופס חתום).
- פיצוי סטטוטורי: **עד 1,000 ש"ח לכל מייל פוגע**, מוענק ללא הוכחת נזק. הפיצויים מצטברים, blast ל-500 נמענים חושף את השולח לחשיפה של 500,000 ש"ח בתביעה ייצוגית.
- קנס פלילי על עבירה ביודעין: **עד 226,000 ש"ח** (נכון ל-2026, מתעדכן שנתית).
- כל הודעת פרסומת חייבת לכלול:
  - את המילה `פרסומת` בתחילת שורת הנושא.
  - את השם המלא של השולח, כתובת מלאה, ופרטי קשר.
  - מנגנון `הסרה` ברור ובולט (קישור unsubscribe או הוראת reply-to-stop), והסרה חייבת להיות מכובדת.
- **חריג**: הודעות תיפעוליות והודעות מערכת ללקוח קיים על מוצר/שירות מאותו סוג שכבר רכש, בתוך 6 חודשים מהרכישה, אינן "דבר פרסומת" - אבל עדיין נדרש מסלול הסרה נפרד.

**כללי bulk sender של Gmail (בתוקף מפברואר 2024, אכיפה הוקשחה מנובמבר 2025):**

אם המשתמש שולח יותר מ-5,000 הודעות ביום לכתובות gmail.com (לדוגמה, blast ניוזלטר מ-`@yourdomain.co.il`):

| דרישה | מה צריך להגדיר |
|-------|----------------|
| SPF | TXT record ב-DNS שמאשר את ה-IPs ששולחים |
| DKIM | TXT record ב-DNS עם המפתח הציבורי, מפתחות חתימה מוגדרים על דומיין השליחה |
| DMARC | TXT record ב-`_dmarc.yourdomain.co.il`, מדיניות לפחות `p=none` למעקב (לעבור ל-`quarantine`/`reject` אחרי audit) |
| RFC 8058 one-click unsubscribe | גם `List-Unsubscribe: <https://...>, <mailto:...>` וגם `List-Unsubscribe-Post: List-Unsubscribe=One-Click` headers |
| שיעור תלונות ספאם | מתחת ל-0.1% (קו אדום ב-0.3%) |
| קישור הסרה גלוי | בגוף המייל |

`gws gmail +send --html` ל-blast מחשבון Workspace כפוף לכל הנ"ל ברגע שעוברים 5,000 ביום לכתובות Gmail. ה-CLI **לא** מוסיף `List-Unsubscribe` headers אוטומטית - צריך לבנות אותם בתוך ה-`--body` HTML או להעביר את ה-blast עצמו ל-ESP טרנזקציוני (Resend, SendGrid, Mailgun).

**זרימת עבודה לפני ניסוח כל הודעה שדומה לשיווק:**

1. לשאול את המשתמש: האם זה תוכן פרסומי (הצעה, מבצע, ניוזלטר)? אם כן, לוודא שיש הסכמה מפורשת בכתב של הנמען בתיק.
2. אם שיווק, להוסיף `פרסומת - ...` בתחילת הנושא ופרטי שולח מלאים + הוראת `הסרה` בגוף.
3. אם הנפח גבוה מ-100 ביום, להמליץ למשתמש להעביר את ה-blast ל-ESP טרנזקציוני במקום `gws gmail +send` (המגבלה היומית של Gmail ונטל האימות לא בנויים לנפח שיווקי).
...
- ב-`gws gmail +send --html`, Gmail ו-Outlook **מסירים את התכונה `dir="rtl"` מתגי `<body>` ו-`<html>`** משיקולי אבטחה. עברית מתיישרת לשמאל אפילו שהיא נקראת ימין-לשמאל. צריך להחיל RTL עם CSS inline על כל אלמנט בלוק: `<p style="direction:rtl; text-align:right;">...</p>`. Apple Mail ו-Yahoo מכבדים `dir="rtl"` נכון, Gmail/Outlook דורשים inline styles.
...
- חוק הספאם הישראלי (סעיף 30א) מטיל פיצוי סטטוטורי של **עד 1,000 ש"ח לכל מייל פוגע**, ללא צורך בהוכחת נזק. הודעות פרסומת חייבות (א) להחזיק הסכמה מפורשת בכתב בתיק, (ב) להתחיל את הנושא ב-`פרסומת`, (ג) לכלול שם מלא + כתובת של השולח, (ד) להציע מנגנון `הסרה` תקין. דילוג על אחד מאלה הוא חשיפה לתביעה ייצוגית.
...
| סעיף 30א (חוק הספאם הישראלי) - סיכום | https://www.kolzchut.org.il/he/פיצוי_בגין_משלוח_דברי_פרסומת_ללא_הסכמה_של_הנמען_(חוק_הספאם) | פיצוי סטטוטורי 1,000 ש"ח לכל מייל פוגע; קנס פלילי 226,000 ש"ח (נכון ל-2026); קידומת נושא `פרסומת` חובה |
| דרישות bulk sender של Gmail | https://support.google.com/a/answer/81126 | SPF + DKIM + DMARC + RFC 8058 one-click unsubscribe לשולחים של ≥5,000 הודעות ביום ל-Gmail; אכיפה הוקשחה נובמבר 2025 |
...
| מגבלות נמענים של Gmail | https://support.google.com/a/answer/166852 | Gmail צרכני 500 נמענים ביום; Workspace 2,000 ביום דרך Gmail (10,000 ביום דרך SMTP relay) |
