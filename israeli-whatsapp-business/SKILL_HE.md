...
**Monday.com + WhatsApp:**
1. תשתמשו בשיטות האינטגרציה של Monday.com (אוטומציות או מחברים של צד שלישי)
2. תפעילו שליחת הודעות WhatsApp בשינוי סטטוס בלוח
3. תתעדו הודעות נכנסות כעדכונים ב-Monday.com
4. תגדירו זרימות עבודה אוטומטיות: "כשסטטוס משתנה ל-X, תשלח תבנית WhatsApp"
...
פעולות:
1. תיצרו תבנית בעברית עם משתנים לשם המרפאה, תאריך ושעה
2. תוסיפו כפתורי תגובה מהירה לאישור/שינוי תור
...
פעולות:
1. תיצרו תבנית אישור הזמנה (בעברית)
2. תיצרו תבנית עדכון משלוח עם קישור מעקב
3. תגדירו webhook לקבלת עדכוני סטטוס מסירה
4. תשלבו עם ספקי משלוחים ישראליים (צ'יטה, HFD, דואר ישראל)
...
פעולות:
1. תבדקו עמידה בתקנות (רשימת הסכמה, חוק הספאם)
2. תיצרו תבנית שיווקית עם פרטי ההצעה
3. תתזמנו לשעות עסקיות מתאימות בישראל
4. תגדירו מעקב אחר שיעורי פתיחה ותגובות
...
- ל-WhatsApp Business API יש חלון שירות לקוחות של 24 שעות. אחרי 24 שעות, אפשר לשלוח רק הודעות תבנית מאושרות מראש. סוכנים עלולים לא להתחשב בכך בעיצוב תהליכי שיחה.
- טקסט עברי במשתני תבנית WhatsApp עלול לשבור את עיצוב ההודעה כשהוא מעורב עם מספרים או אנגלית. תשתמשו בתווי Unicode isolate (U+2066/U+2069) סביב תוכן מעורב כיוונים.

## פתרון בעיות

### שגיאה: "התבנית נדחתה"
סיבה: התבנית מפרה את מדיניות WhatsApp או בעיות עיצוב
פתרון: תוודאו שהטקסט בעברית מעוצב כראוי, בלי תוכן אסור (הימורים, מבוגרים), שהמשתנים כוללים דוגמאות ושהקטגוריה תואמת את סוג התוכן.

### שגיאה: "שליחת ההודעה נכשלה"
סיבה: מגוון -- מספר לא תקין, המשתמש לא בוואטסאפ, חריגה ממגבלת קצב
פתרון: תוודאו פורמט 972+, תבדקו שלמשתמש יש WhatsApp, תכבדו את מגבלות הקצב (80 הודעות/שנייה ב-Cloud API).

### שגיאה: "ה-webhook לא מקבל הודעות"
סיבה: כתובת webhook לא אומתה או אפליקציית Meta לא מוגדרת
פתרון: תוודאו שכתובת ה-webhook היא HTTPS, שטוקן האימות תואם ושאפליקציית Meta רשומה ל-webhook של הודעות.