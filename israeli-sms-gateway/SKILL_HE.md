...
def send_sms_sms4free(to: str, message: str, api_key: str,
    ...
    # הערה: נקודת הקצה המקורית של SMS4Free אינה זמינה יותר
    # צרו קשר עם SMS4Free לקבלת תיעוד API ונקודות קצה עדכניות
    payload = {
        "key": api_key,
        "user": username,
        "pass": pass_key,
    ...
    # הערה: שמרו פרטי התחברות במשתני סביבה, לא בקוד
    # בדקו עם SMS4Free לקבלת נקודת הקצה הנוכחית
    print("צרו קשר עם SMS4Free לקבלת נקודת קצה ותיעוד עדכניים")
```
...
פעולות:
1. אמת את כל מספרי הטלפון ברשימה (נרמל לפורמט +972)
2. בדק התאמה למאגר "אל תפריע" ונתוני הסכמה
3. יישם תקנות ספאם ישראליות: כלול זיהוי שולח, מנגנון ביטול
4. תזמן לשעות עסקים ישראליות (לא בשבת/חגים)
5. שלח דרך API המוני עם מעקב מסירה
...
### הפניות
- `references/israeli-sms-compliance.md` — דרישות חוק ספאם ישראלי לשיווק SMS: כללי הסכמה מפורשת, תהליך בדיקה במאגר "אל תפריע", מנגנוני ביטול נדרשים, שעות שליחה מותרות ומבנה עונשים על הפרות (עד 1,000 שח לכל הודעה לא רצויה). פנו אליו כשמקימים קמפיינים מסחריים או בודקים התאמה לחוק.
- `references/provider-api-docs.md` — סיכומי תיעוד API לספקי SMS ישראליים (SMS4Free, InforUMobile) ולספקים בינלאומיים עם תמיכה ישראלית (Twilio, Vonage), כולל אימות, כתובות נקודות קצה, קודי תגובה, דוחות מסירה וקידוד תווים עבריים (GSM-7 לעומת UCS-2 לעברית). פנו אליו כשמשלבים עם ספק מסוים או פותרים בעיות מסירה.
...
פתרון: השתמשו בקידוד UCS-2 להודעות בעברית. שימו לב שהודעות מעל 70 תווים עבריים מתחלקות ל-SMS רב-חלקי (67 תווים לסגמנט). תקצבו בהתאם לעלויות SMS המוני. חלק מהספקים מטפלים בקידוד אוטומטית.